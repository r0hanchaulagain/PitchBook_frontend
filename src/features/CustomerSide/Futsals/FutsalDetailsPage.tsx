...
const LeafletMap = lazy(() => import("@featureComponents/LeafletMap"));
...
const FutsalDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    apiQuery<{ bookedSlots: { startTime: string; endTime: string }[] }>(
      `booking/available-slots?futsalId=${id}&date=${selectedDate}`
    )
  ...
  useEffect(() => {
    ...
    console.log('selectedStartTime:', selectedStartTime && selectedStartTime.format('HH:mm'));
    console.log('selectedEndTime:', selectedEndTime && selectedEndTime.format('HH:mm'));
    ...
    try {
      const res = await apiMutation({
        ...
        body: {
          futsalId: futsal._id,
          date: selectedDate,
          startTime: selectedStartTime ? selectedStartTime.format("HH:mm") : undefined,
          endTime: selectedEndTime ? selectedEndTime.format("HH:mm") : undefined,
          bookingType: "full",
          teamA: true,
          teamB: true,
        },
      ...
      const errorMsg = e?.response?.data?.message || e.message || "Booking failed";
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      const res = await apiMutation({
        method: "POST",
        endpoint: `booking/${booking._id}/initiate-payment`,
        body: {
      ...
      const errorMsg = e?.response?.data?.message || e.message || "Payment initiation failed";
  ...
  function handleStartTimeAccept(val: Dayjs | null) {
    console.log('handleStartTimeAccept called with:', val && val.format('HH:mm'));
    if (!val || !selectedEndTime || !minTime || !maxTime) return;
    if (val.isBefore(minTime) || val.isAfter(maxTime) || val.isAfter(selectedEndTime)) {
    ...
    if (isOverlap(val, selectedEndTime)) {
      toast.error("Selected slot overlaps with a booked slot. Please choose a different time.");
  ...
  function handleEndTimeAccept(val: Dayjs | null) {
    ...
    if (val.isAfter(maxTime) || val.isBefore(minTime) || val.isBefore(selectedStartTime)) {
  ...
  function isSlotValid() {
    ...
    const duration = Math.abs(selectedEndTime.diff(selectedStartTime, 'minute'));
    return [30, 60, 90, 120].includes(duration);
  }

  // Helper to get the final price for the selected slot
  function getFinalPrice() {
    if (!selectedStartTime || !selectedEndTime) return 0;
    const durationMinutes = Math.abs(selectedEndTime.diff(selectedStartTime, 'minute'));
    const hours = durationMinutes / 60;
    return Math.round(futsal.pricing?.finalPrice * hours);
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[70vh] bg-muted">
        <div className="bg-card shadow-lg rounded-lg p-10 flex flex-col items-center w-full max-w-md">
          <img src="/logo.svg" alt="Soccerslot Logo" className="w-20 h-20 mb-4" />
          <h2 className="text-2xl font-bold mb-2 text-primary">Futsal Not Found</h2>
          <p className="text-muted-foreground mb-6 text-center">Sorry, we couldn't find the futsal you were looking for. It may have been removed or the link is incorrect.</p>
          <Button className="w-full" onClick={() => navigate('/futsals')}>Back to Futsals</Button>
        </div>
      </div>
    );
  }
    ...
    <main className="px-18 mx-auto p-5">
      <div className="flex flex-col">
        <div className="flex justify-between items-start my-5">
          <div>
            <h1 className="text-xl font-semibold text-primary">{futsal.name}</h1>
            <p className="text-sm text-muted-foreground">
              {futsal.location?.address || futsal.location?.city || "Location not available"}
              <span className="text-secondary ml-2 flex items-center"><Star size={16} /> {futsal.pricing?.avgRating || "-"} ({futsal.pricing?.reviewCount || 0} ratings)</span>{" "}
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" className="">Share</Button>
            <Button className=" text-primary-foreground">Book Now</Button>
            <Button variant="outline" className="">Bulk / Corporate</Button>
          </div>
        </div>
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            <div className="mb-4">
              <div className="relative w-full h-[68vh] mb-2 flex items-center justify-center">
                    ...
                    {futsal.images.map((img: string, idx: number) => (
                      <img
                        ...
                        className={`h-12 w-20 object-cover rounded cursor-pointer border-2 transition-all ${
                          idx === currentImageIdx ? 'border-accent shadow-lg' : 'border-transparent opacity-70 hover:opacity-100'
                        }`}
                        onClick={() => {
                          if (idx !== currentImageIdx) {
                            setTransitionDirection(idx > currentImageIdx ? 'right' : 'left');
                ...
                {Array.isArray(futsal.images) && futsal.images.length > 1 && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setImgTransition(true);
                      setTimeout(() => {
                        setCurrentImageIdx((idx) => (idx === 0 ? futsal.images.length - 1 : idx - 1));
                        setImgTransition(false);
                      }, 300);
                    }}
                  >
                ...
                <div className="relative w-full h-[68vh] overflow-hidden">
                  <img
                    src={Array.isArray(futsal.images) && futsal.images.length > 0 ? futsal.images[currentImageIdx] : "/images/futsals_page/search_img.jpg"}
                alt="Futsal Court"
                ...
                {Array.isArray(futsal.images) && futsal.images.length > 1 && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setImgTransition(true);
                      setTimeout(() => {
                        setCurrentImageIdx((idx) => (idx === futsal.images.length - 1 ? 0 : idx + 1));
                        setImgTransition(false);
                      }, 300);
                    }}
                  >
            ...
            <Card className="p-4 mb-4 bg-popover">
              <h2 className="text-lg font-semibold text-primary mb-4">About Venue</h2>
              <p className="text-sm text-muted-foreground">
                {futsal.info || "No description available."}
              </p>
            </Card>
            <div className="bg-popover p-4 rounded-md shadow-md">
              <h2 className="text-xl font-semibold text-primary mb-4">Book Your Slot</h2>
              {/* Date Picker and Selected Date */}
              <div className="flex items-center gap-4 mb-9 flex-wrap">
                <Label className="text-sm text-muted-foreground font-bold">Select Date:</Label>
                <Input
                  type="date"
                  className="w-1/3"
                  value={selectedDate}
              ...
              {/* Occupied Slots */}
              <div>
                <h3 className="font-semibold mb-4 text-md text-primary">Occupied Slots</h3>
                {slotLoading ? (
                  <div className="text-muted-foreground text-sm">Loading...</div>
                ) : bookedSlots.length === 0 ? (
                  <div className="text-muted-foreground text-sm">No slots booked for this date.</div>
                ) : (
              ...
              {/* Select Your Slot */}
              <div className="my-4">
                <h3 className="font-bold mb-4 text-lg text-primary text-center">Select Your Slot</h3>
                <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
                  <div className="flex flex-col items-center">
                    <span className="mb-2 font-medium text-primary">Start time:</span>
                    
                      <TimePicker value={selectedStartTime} onChange={handleStartTimeChange} onAccept={handleStartTimeAccept} minTime={minTime} maxTime={maxTime} />

                  </div>
                  <div className="flex flex-col items-center">
                    <span className="mb-2 font-medium text-primary">End time:</span>
                      <TimePicker value={selectedEndTime} onChange={handleEndTimeChange} onAccept={handleEndTimeAccept} minTime={minTime} maxTime={maxTime} />
                  </div>
                </div>
              </div>
              {/* Book Now Button */}
  ...
  <CardContent className="p-0">
          <div className="flex flex-col gap-4">
            <Card className="bg-popover p-4 gap-2">
              <h2 className="text-lg font-semibold text-primary mt-4 mb-2">Timing</h2>
              <div className="space-y-1 mb-4">
                {futsal.operatingHours ?
                  Object.entries(futsal.operatingHours).map(([day, hours]: any) => {
                    const todayIdx = new Date().getDay();
                    const days = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
                    const isToday = days[todayIdx] === day.toLowerCase();
                    return (
                      <div key={day} className={`flex gap-2 items-center rounded p-1.5 ${isToday ? 'bg-accent font-bold text-accent-foreground' : ''}`}>
                        <span className="font-medium w-24 capitalize">{day}:</span>
                        <span className="font-mono text-muted-foreground">{hours.open} - {hours.close}</span>
                        {isToday && <span className="ml-2 px-2 py-0.5 rounded bg-primary text-xs">Today</span>}
                      </div>
                    );
                  }) : <span className="text-muted-foreground">N/A</span>}
              </div>
              <h2 className="text-lg font-semibold text-primary mt-4 mb-2">Amenities</h2>
              <div className="flex flex-wrap gap-2 mt-2">
                {(futsal.amenities || []).filter((a: string) => a !== "basic futsal facilities").map((a: string) => (
                  <Badge key={a} className="text-sm font-medium px-3 py-1">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {futsal.pricing?.rules?.map((rule: any) => {
                    const todayIdx = new Date().getDay();
                    const days = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
                    const isToday = days[todayIdx] === rule.day.toLowerCase();
                    return (
                      <tr key={rule._id} className={isToday ? 'bg-accent font-bold text-accent-foreground' : ''}>
                        <td className="p-2">{rule.day.charAt(0).toUpperCase() + rule.day.slice(1)}{isToday && <span className="ml-2 px-2 py-0.5 rounded bg-primary text-xs">Today</span>}</td>
                        <td className="p-2">{rule.price.toLocaleString()}</td>
                  </tr>
                    );
                  })}
                </tbody>
              </table>
              <span>Today's Price: Rs. {futsal.pricing?.finalPrice?.toLocaleString()}</span>
            </div>
            <div className="bg-popover p-4 rounded-md shadow-md">
              <h2 className="text-lg font-semibold text-primary mb-2">Location</h2>
              <p className="text-sm text-muted-foreground mb-2">
                {futsal.location?.address || futsal.location?.city || "Location not available"}
              </p>
              {/* Embed map preview with OpenStreetMap */}
              {futsal.location?.coordinates?.coordinates && !showMapModal && (
                <Suspense fallback={<div style={{height: 200, width: '100%'}}>Loading map...</div>}>
                  <LeafletMap
                    center={[futsal.location.coordinates.coordinates[1], futsal.location.coordinates.coordinates[0]]}
                    zoom={16}
                    height={200}
                    width={'100%'}
            ...
            <div className="bg-popover p-4 rounded-md shadow-md">
              ...
              <p className="text-sm text-muted-foreground flex items-center"><Phone size={16} className="mr-2" /> {futsal.contactInfo?.phone || "N/A"}</p>
              <p className="text-sm text-muted-foreground mt-2">{futsal.location?.city || "N/A"}</p>
            </div>
          </div>
  </CardContent>
</Card>
        ...
        <DialogContent className="sm:max-w-[425px] z-50">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                <strong>Time Slot:</strong> {selectedStartTime && selectedStartTime.format('HH:mm')} - {selectedEndTime && selectedEndTime.format('HH:mm')}
              </p>
              <p className="text-sm text-muted-foreground">
                <strong>Price:</strong> Rs. {booking?.price?.toLocaleString() || booking?.price || '-'}
              </p>
            </div>
            <div className="my-5">
              ...
              <Button
                ...
                disabled
              >
                <img src="https://avatars.githubusercontent.com/u/31564639?s=280&v=4" className="size-8" />
                Khalti
              </Button>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button className=" text-primary-foreground" onClick={handleProceedToPayment} disabled={paymentLoading}>
              Proceed to Payment
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80">
          <div className="bg-card rounded-lg shadow-lg p-4 max-w-3xl w-full relative">
            ...
            {futsal.location?.coordinates?.coordinates && (
              <Suspense fallback={<div style={{height: 400, width: '100%'}}>Loading map...</div>}>
                <LeafletMap
                  center={[futsal.location.coordinates.coordinates[1], futsal.location.coordinates.coordinates[0]]}
                  zoom={16}
                  height={400}

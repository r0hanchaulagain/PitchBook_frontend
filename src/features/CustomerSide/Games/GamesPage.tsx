...
export default function GamesPage() {
  ...
  const fetchPartialBookings = useCallback(async (location?: { lat: number; lng: number }, searchRadius = radius) => {
    try {
      setLoading(true);
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {showRadiusSlider && userLocation && (
            <div className="flex items-center gap-3 ml-2 w-48">
              <span className="text-sm whitespace-nowrap">{sliderValue} km</span>
              <Slider
                value={[sliderValue]}
                max={50}
                min={1}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
            ...
            <Card key={booking._id}>
              <CardHeader>
                <CardTitle className="capitalize">
                  {typeof booking.futsal === 'string' ? 'Futsal Court' : booking.futsal.name}
                </CardTitle>
                <CardDescription>
              ...
              <CardContent>
                <div className="space-y-2">
                  {typeof booking.futsal === 'string' ? (
                    <p className="text-sm">
                      <span className="font-medium">Futsal ID:</span> {booking.futsal}
                    </p>
                  ) : (
                    <>
                      <p className="text-sm">
                        <span className="font-medium">Location:</span> {booking.futsal.location?.address || 'N/A'}
                      </p>
                      <p className="text-sm">
                        <span className="font-medium">City:</span> {booking.futsal.location?.city || 'N/A'}
                      </p>
                    </>
                  )}
                  <p className="text-sm">
                    <span className="font-medium">Price:</span> Rs. {booking.price || 'N/A'}
                  </p>
                  <p className="text-sm">
                    <span className="font-medium">Type:</span> {booking.bookingType === 'partial' ? 'Looking for players' : 'Full booking'}
                  </p>
                </div>
              </CardContent>
              <CardFooter>
                ...
                >
                  {booking.status === 'pending' ? 'Join Game' : 'Already Joined'}
                </Button>
              </CardFooter>
            </Card>
          ))
          ...
          <div className="col-span-full text-center py-12">
            <p className="text-muted-foreground">No game sessions found. Be the first to host one!</p>
          </div>
        )}
      </div>
    </div>
